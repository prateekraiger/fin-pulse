...
import { cn, getFYOptions, getCurrentFY } from "@/lib/utils";
...
export function AppShell({ children, currentPath = "/" }) {
  ...
  const activePath = currentPath !== "/" ? currentPath : location.pathname;
  const [fy, setFy] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("fy") || getCurrentFY();
    }
    return getCurrentFY();
  });
  ...
  const handleFyChange = useCallback(
    (newFy) => {
      ...
      // Invalidate all queries so they refetch with the new FY
      // This is much better than window.location.reload()
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-t border-zinc-100">
        <div className="flex items-center gap-3">
          ...
          <Link to="/onboarding" title="Settings">
            <Settings
              size={14}
    ...
    <div className="flex h-screen bg-zinc-50 overflow-hidden font-sans">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <div className="md:hidden flex items-center gap-3 px-4 py-3 bg-white border-b border-zinc-200">
          ...
          <span className="text-sm font-bold text-zinc-900">FinPulse</span>
        </div>
...
/**
 * Hook to read the current FY from AppShell context.
 * Falls back to localStorage / getCurrentFY() when used outside AppShell.
 */
export function useFY() {
  const contextFY = useContext(FYContext);
  if (contextFY) return contextFY;
  if (typeof window !== "undefined") {
    return localStorage.getItem("fy") || getCurrentFY();
  }
  return getCurrentFY();
